 ...
 *  @version $Id: CRANELegacyThread.java,v 1.3 2005/01/30 15:13:41 ilyad Exp $
 *  @see #main
 */
public class CRANELegacyThread extends Thread
{
    /// Hard-coded configuration: how long to wait for a response from UDP version negotiation request.
    private final int defaultConnectTimeout = Constants.DEFAULT_CONNECT_TIMEOUT;

    /// Hard-coded configuration: how long to wait before trying to reconnect to a transmitter.
    private final int defaultReconnectTimeout = Constants.DEFAULT_RECONNECT_INTERVAL;
    ...
    public void main(String [] arg) {
        CRANELegacyThread test = new CRANELegacyThread(arg);
        test.run();
    }
     ...
     */
    public CRANELegacyThread(String [] arg) {
        if (arg.length != 3) {
